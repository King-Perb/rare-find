...
export class RapidAPIAmazonClient {
  ...
  constructor(config: RapidAPIConfig, httpClient: IHttpClient) {
    this.config = config;
  ...
  async getItemById(marketplaceId: string): Promise<MarketplaceListing | null> {
    ...
    try {
      const response = await this.httpClient.fetch(`${BASE_URL}/product-details?${params}`, {
        method: 'GET',
  ...
  async search(params: MarketplaceSearchParams): Promise<MarketplaceSearchResult> {
    ...
    try {
      const response = await this.httpClient.fetch(`${BASE_URL}/search?${searchParams}`, {
        method: 'GET',
        headers: this.getHeaders(),
      });
