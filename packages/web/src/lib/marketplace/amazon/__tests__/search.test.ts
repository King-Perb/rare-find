...
describe('searchAmazon', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(createAmazonClient).mockReturnValue(mockClient as never);
  });
  ...
  it('should call client.search with params when no preferences', async () => {
    ...
    expect(createAmazonClient).toHaveBeenCalled();
  ...
  it('should apply preference filters when preferences are provided', async () => {
    ...
    await searchAmazon(params, preferences);
