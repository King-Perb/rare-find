...
import type { ICryptoProvider, IHttpClient } from '../../../interfaces';
...
export class AmazonClient {
  ...
  private createSignature(
    ...
    const payloadHash = this.cryptoProvider.sha256(payload);
    ...
    const stringToSign = [
      ...
      timestamp,
      credentialScope,
      this.cryptoProvider.sha256(canonicalRequest),
    ].join('\n');

    // Step 3: Calculate signature
    const kDate = this.cryptoProvider.hmacSha256(`AWS4${this.credentials.secretKey}`, dateStamp);
    const kRegion = this.cryptoProvider.hmacSha256(kDate, this.region);
    const kService = this.cryptoProvider.hmacSha256(kRegion, 'ProductAdvertisingAPI');
    const kSigning = this.cryptoProvider.hmacSha256(kService, 'aws4_request');
    const signatureBuffer = this.cryptoProvider.hmacSha256(kSigning, stringToSign);

    // Convert signature buffer to hex string
    const signatureHex = signatureBuffer.toString('hex');

    // Step 4: Build authorization header
    const authorization = `${algorithm} Credential=${this.credentials.accessKey}/${credentialScope}, SignedHeaders=${signedHeaders}, Signature=${signatureHex}`;
  ...
  async search(params: MarketplaceSearchParams): Promise<MarketplaceSearchResult> {
    ...
    try {
      ...
      const response = await this.httpClient.fetch(this.endpoint, {
        method: 'POST',
        headers,
        body: payload,
      });
  ...
  async getItemById(marketplaceId: string): Promise<MarketplaceListing | null> {
    ...
    try {
      ...
      const response = await this.httpClient.fetch(this.getItemsEndpoint, {
        method: 'POST',
        headers,
        body: payload,
      });
