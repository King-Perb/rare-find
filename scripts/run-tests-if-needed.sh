...
# Works for all packages (web, shared, mobile) via turbo
...
if ! git rev-parse --verify "$upstream_branch" >/dev/null 2>&1; then
  ...
  echo "New branch detected, running tests for all packages"
  npm run test
  test_result=$?
...
if [ "$has_code_changes" = true ]; then
  # Code changes detected, run tests for all packages
  echo "Code changes detected, running tests for all packages (web, shared, mobile)"
  npm run test
  test_result=$?
  if [ $test_result -eq 0 ]; then
