...
- Q: How should entity types be structured given that Supabase autogenerated types exist in `packages/web/src/lib/db/types.ts`? → A: Create simple type aliases/helpers in `packages/web/src/lib/db/types.ts` that export convenient names (e.g., `export type Listing = Database['public']['Tables']['listings']['Row']`) rather than creating separate type files or using verbose Supabase types directly throughout the codebase.
