...
if ! git rev-parse --verify "$upstream_branch" >/dev/null 2>&1; then
  ...
  echo "New branch detected, running build for all packages"
  npm run build
  build_result=$?
...
if [ "$has_code_changes" = true ]; then
  # Code changes detected, run build for all packages
  echo "Code changes detected, running build for all packages (web, shared, mobile)"
  npm run build
  build_result=$?
  if [ $build_result -eq 0 ]; then
